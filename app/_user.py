...
@router.callback_query(F.data.startswith('day_'), Reserve.day)  # получение ответа на выбор дня недели
async def get_service_4(callback: CallbackQuery, state: FSMContext):
    await callback.answer('День недели выбран.')
    await state.update_data(day=callback.data.split('_')[1])  # обновилось состояние выбора дня недели
    await state.set_state(Reserve.hour) #открыто состояние выбора времени
    await callback.message.answer('Выберите время', reply_markup=await kb.hours())  # предлагается выбрать время
